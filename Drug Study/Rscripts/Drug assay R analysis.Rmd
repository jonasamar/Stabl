...
```{r}
Drugs <- c( #This vector contains the name of the drugs in the order corresponding to the plate number they were tested
  "Cefotaxime",
  "Lansoprazole",
  "Iopamidol",
  "Iohexol",
...
for (i in seq(1, 15)){ #Plate values are going from 1 to 15
    MyData = MyData %>%
    mutate(Drug =  ifelse(Plate == i, Drugs[i], Drug))
}

MyData = subset(MyData, select = -c(Plate)) #Now that the Plate contained in information is transfered in the Drug column, we can remove column Plate
```
...
for(dosepoint in unique(MyData$dose)){
  ...
  # Removing the median column & Stimulated - Unstimulated
...
Doseresponse = final_data %>%
  ...
  dcast(population + reagent +drug + stimulation ~ dose, value.var = "feature") %>% # dcast(new rows ~ new columns, values)
  # mutate(dose_response = case_when(D1 > D10 & D10 > D100   ~ T,
  #                            TRUE ~ F)) %>%
...
InhibitionRatio <- Doseresponse %>% group_by(drug, dose_response) %>%
  dplyr::summarise(N = n()) %>% # calculate the number of reagent and stimulation where it has the dose_response behavior
  arrange(desc(N)) %>%
  dcast(drug ~ dose_response, value.var = "N") %>%
  mutate(total = `FALSE`+ `Activation` + `Inhibitor`) %>%
  mutate(prop = `Inhibitor` / total*100)%>% # calculate the % of the time the drug behave as an inhibitor
  arrange(desc(prop))
...
Potency = Doseresponse %>% 
  ...
  mutate(x=name) %>% 
  nest() %>% 
  mutate(model = map(data, ~ lm(value ~ x, data = .)), # linear model fitting
         result = map(model, function(x) list(intercept= x$coef[1], # summary of the linear model
                                              slope = x$coef[2],
...
mat <- Potency %>%
  dplyr::filter(name == "D10") %>% # choosing one arbitrary dose (all the doses are part of the same model)
  ...
  tidyr::unite(rowname, drug, population) %>% #concatenate the metadata
  tibble::column_to_rownames() %>%
  t()
mat[is.na(mat)] <- 0 # Replacing missing values with 0
...
for(drug in Drugs){
  ...
  score <- as.numeric(featuretable$Max.Proba)*as.numeric(unlist(cal)) #feature importance * drug effect on the feature
...
for(drug in Drugs){
  cal = featuretable %>% dplyr::select(c(drug))
  score <- as.numeric(featuretable$coefficient)*as.numeric(unlist(cal)) #feature importance * drug effect on the feature
  score_f <- sum(abs(score)*100)
...
new.row \<- data.frame(drug = i, score.ina = score.ina, score.model = score.model) summary.score \<- rbind(summary.score, new.row)}
